...
{     function __construct() 
    ...
    { 
        ...
        //jika usernya ada         if ($user) { 
            //jika user sudah aktif             if ($user['is_active'] == 1) { 
                //cek password                 if (password_verify($password, $user['password'])) { 
                    $data = [ 
                        'email' => $user['email'], 
                    ...
                    $this->session-
>set_flashdata('pesan', '<div class="alert alert-danger alertmessage" role="alert">Password salah!!</div>');                     redirect('home'); 
                } 
            } else { 
