...
class Booking extends CI_Controller
{
    ...
    { 
        ...
        $data['items'] = $this->db->query("select*from booking book, booking_detail dt, buku bku where dt.id_booking=book.id_booking and dt.id_buku=bku.id and book.id_user='$where'")->result_array();
    ...
    { 
        ...
        $data['useraktif'] = $this->ModelUser->cekData(['id' => $this->session->userdata('id_user')])->result(); 
        $data['items'] = $this->db->query("select*from booking book, booking_detail dt, buku bku where dt.id_booking=book.id_booking and dt.id_buku=bku.id and book.id_user='$id_user'")->result_array(); 
         
        $this->load->library('pdf');

        $this->pdf->setPaper('A4', 'potrait');
        $this->pdf->filename = "bukti-booking-$id_user.pdf";
        $this->pdf->load_view('booking/bukti-pdf.php', $data); 
    }
}